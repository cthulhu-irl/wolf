...
static boost::leaf::result<std::tuple<w_av_packet, w_av_config, w_av_config>>
         ...
         _In_ const bool p_flush = true)
{
  ...
  auto _dst_config = w_av_config(AVPixelFormat::AV_PIX_FMT_YUV420P,
  ...
  BOOST_LEAF_AUTO(_yuv_frame,
                  _src_frame.convert_video(std::move(_dst_config_clone)));
    ...
    _In_ const bool p_flush = false)
{
  ...
  auto _decoded_frame =
      w_av_frame(std::forward<w_av_config &&>(std::get<2>(p_encoded_tuple)));
