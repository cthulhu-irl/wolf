...
boost::leaf::result<std::vector<std::byte>> w_lzma::compress_lzma1(
...
boost::leaf::result<std::vector<std::byte>> w_lzma::compress_lzma2(
    _In_ const gsl::span<const std::byte> p_src, _In_ uint32_t p_level) {
  const auto _src_size = gsl::narrow_cast<int>(p_src.size());
  if (_src_size == 0) {
    return W_FAILURE(std::errc::invalid_argument, "the source is empty");
...
boost::leaf::result<std::vector<std::byte>> w_lzma::decompress_lzma1(
...
boost::leaf::result<std::vector<std::byte>> w_lzma::decompress_lzma2(
