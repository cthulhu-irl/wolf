...
/**
 * @brief make a default instantiable delter callable out of compile-time known
 * callable objects.
 *
...
struct c_type_deleter {
  ...
  constexpr auto operator()(pointer p_ptr) noexcept { return Func(p_ptr); }
 ...
 */
inline auto dbconn_error_check(const dbconn& p_conn)
 ...
 * @return error or nothing.
 */
inline auto dbresult_error_check(const dbresult& p_res)
    -> boost::leaf::result<void> {
  if (!p_res) {
    return W_FAILURE(std::errc::invalid_argument, "result is null.");
  }
  ...
  switch (PQresultStatus(p_res.get())) {
    ...
    case PGRES_BAD_RESPONSE:
      return W_FAILURE(PQresultStatus(p_res.get()),
                       PQresultErrorMessage(p_res.get()));
...
inline auto to_string(std::string&& p_value) -> std::string { return p_value; }

template <typename T>
  requires std::is_integral_v<T>
inline auto from_string(T* p_ptr, const char* p_str)
    -> boost::leaf::result<void> {
  auto [errptr, errcode] =
      std::from_chars(p_str, p_str + strlen(p_str), *p_ptr);
  if (errcode != std::errc()) {
    return W_FAILURE(errcode,
                     "could not convert given string to given integral type.");
  }

  return {};
}

inline auto from_string(std::string* p_ptr, const char* p_str)
    -> boost::leaf::result<void> {
...
class w_pgdb_record {
 ...
 public:
  ...
  auto at(std::size_t p_field_index) const noexcept -> const char* {
    return PQgetvalue(_res_ptr.get(), static_cast<int>(_record_index),
                      static_cast<int>(p_field_index));
  }
 ...
 private:
  ...
  template <typename T>
  auto scan_field_into(std::size_t p_field_index, T* p_ptr)
      -> boost::leaf::result<void> {
  ...
  w_pgdb_record(const internal::dbresult& p_res, std::size_t p_index) noexcept
      : _res_ptr(p_res), _record_index(p_index) {}
...
class w_pgdb_queryresult {
 ...
 public:
   ...
   *
   * @param p_handler callable to pass each record to and returns a boost leaf
   * result.
  ...
  auto cancel() -> boost::leaf::result<void> {
    ...
    if (!cancel_req) {
      return W_FAILURE(PQstatus(_conn_ptr.get()),
                       "could not create cancellation object.");
    }

    // stack is much faster and safer, heap alloc isn't required if we don't hit
    // error.
 ...
 private:
  explicit w_pgdb_queryresult(internal::dbconn& p_conn,
                              internal::dbresult&& p_result)
      : _conn_ptr(p_conn), _res_ptr(std::move(p_result)) {}
...
class w_pgdb_execresult {
 ...
 public:
  ...
  w_pgdb_execresult(w_pgdb_execresult&& p_other) noexcept
      : _conn_ptr(p_other._conn_ptr), _res_ptr(p_other._res_ptr.release()) {}
 ...
 private:
  explicit w_pgdb_execresult(internal::dbconn& p_conn,
                             internal::dbresult&& p_res)
      : _conn_ptr(p_conn), _res_ptr(std::move(p_res)) {}
...
class w_pgdb_conection {
 public:
   ...
   * @note valid param details can be found at:
   * https://www.postgresql.org/docs/current/libpq-connect.html
   *
   * @param p_params pairs of key-value parameters. (e.g. {"host", "localhost"},
   * {"port", "26257"})
   * @return database connection or error.
   */
  template <typename... Ts, typename... Us>
  static auto make(const std::pair<Ts, Us>&... p_params)
      -> boost::leaf::result<w_pgdb_conection> {
    const auto params_count = sizeof...(Ts);

    const char* keywords[params_count + 1] = {
        static_cast<const char*>(p_params.first)..., nullptr};
    const char* values[params_count + 1] = {
        static_cast<const char*>(p_params.second)..., nullptr};
   ...
   * @return database connection or error.
   */
  static auto make(const char* p_conn_str)
      -> boost::leaf::result<w_pgdb_conection> {
   ...
   *
   * @note params will be converted to string by an internal converter for
   * simple types. if your type isn't supported, convert to string then pass it
   * to this method.
   *
  ...
  auto execute(const char* p_sqlstr, Args&&... p_params)
      -> boost::leaf::result<w_pgdb_execresult> {
    ...
    if constexpr (arg_count == 0) {
      if (PQsendQuery(_conn_ptr.get(), p_sqlstr) <= 0) {
        return W_FAILURE(PQstatus(_conn_ptr.get()),
                         PQerrorMessage(_conn_ptr.get()));
      }
    } else {
      // currently raw data isn't supported, and it's only text/string.
      // IIFE for compile-time stack array allocation and loop-free
      // initialization.
      int res = [&](std::same_as<std::string> auto&&... p_str_params) {
        const char* string_array[arg_count] = {(p_str_params.c_str())...};

        int length_array[arg_count] = {
            static_cast<int>(p_str_params.size())...};

        return PQsendQueryParams(
            _conn_ptr.get(), p_sqlstr, arg_count,
            nullptr,  // infer types, they're not determined.
            string_array, length_array,
            nullptr,  // only nul-terminated c strings (text), binary isn't
                      // supported.
            0);
    ...
    // otherwise the connection will still hold the result and next execute
    // won't work.
    auto dbres = w_pgdb_execresult(
        _conn_ptr, internal::dbresult(PQgetResult(_conn_ptr.get())));
   ...
   */
  template <typename... Args>
  auto query(const char* p_sqlstr, Args&&... p_args)
      -> boost::leaf::result<w_pgdb_queryresult> {
    BOOST_LEAF_AUTO(res, execute(p_sqlstr, std::forward<Args>(p_args)...));
    return std::move(res).as_query();
  }

  /**
   * @brief execute the given sql query with given sql parameters in streaming
   * mode.
   *
   ...
   */
  template <typename... Args>
  auto stream_query(const char* p_sqlstr, Args&&... p_args)
      -> boost::leaf::result<w_pgdb_queryresult> {
    BOOST_LEAF_AUTO(res, execute(p_sqlstr, std::forward<Args>(p_args)...));
 ...
 private:
  explicit w_pgdb_conection(internal::dbconn&& p_conn)
      : _conn_ptr(std::move(p_conn)) {}
